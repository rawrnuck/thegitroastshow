...
const RoastScript = ({
  ...
  const startTypingAnimation = useCallback(
    (text: string, duration: number): Promise<void> => {
      return new Promise((resolve) => {
        ...
        const baseDelay = Math.max(25, duration / (totalChars * 1.0));
  ...
  const playTTS = useCallback(
    (text: string): Promise<{ onAudioStart: Promise<void>; onComplete: Promise<void> }> => {
      return new Promise((resolve) => {
        if (!ttsEnabled) {
          console.log("TTS is disabled, skipping speech");
          resolve({
            onAudioStart: Promise.resolve(),
        ...
        let audioStartResolve: (() => void) | null = null;
        let audioCompleteResolve: (() => void) | null = null;

        const onAudioStart = new Promise<void>((resolveStart) => {
          audioStartResolve = resolveStart;
        });

        const onComplete = new Promise<void>((resolveComplete) => {
          audioCompleteResolve = resolveComplete;
        });
        ...
        const onSpeechEnd = () => {
          ...
          document.removeEventListener("tts-error", speechErrorListener);
        ...
        const speechErrorListener = () => {
          console.warn("TTS error occurred");
          if (!speechStarted) {
            onSpeechStart(); // Trigger start even on error to prevent hanging
          }
          onSpeechEnd();
        };
        ...
        document.addEventListener("tts-error", speechErrorListener);
        ...
        // Fallback timeout for audio start (in case tts-started event doesn't fire)
        setTimeout(() => {
          if (!speechStarted) {
            console.warn("TTS start fallback timeout triggered");
            onSpeechStart();
          }
        }, 2000); // 2 second fallback for audio start
        ...
        resolve({ onAudioStart, onComplete });
      });
    },
  ...
  const runSequence = useCallback(async () => {
    ...
    try {
      ...
      if (currentItem.type === "sound") {
        ...
        console.log(
          "Starting ElevenLabs TTS request:",
        ...
        try {
          ...
          // Start TTS if enabled and wait for audio to begin
          if (soundEnabled && ttsEnabled) {
            const ttsResult = await playTTS(speechItem.text);
            
            // Wait for TTS audio to actually start playing
            console.log("Waiting for TTS audio to start...");
            await ttsResult.onAudioStart;
            
            console.log("TTS audio started, beginning typing animation");
            
            // Now start typing animation synchronized with audio
            promises.push(startTypingAnimation(textToShow, typingDuration));
            
            // Wait for TTS to complete
            promises.push(ttsResult.onComplete);
          } else {
            // If TTS is disabled, start typing animation immediately
            console.log("TTS disabled, starting typing animation immediately");
            promises.push(startTypingAnimation(textToShow, typingDuration));
          }
          ...
          promises.push(
            new Promise((resolve) =>
              setTimeout(resolve, Math.max(2000, typingDuration * 1.8))
            )
          );
