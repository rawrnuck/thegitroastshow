...
const FLOW_STEPS = [
  ...
  "VIDEO_INTRO", // Video intro that pauses at 9s with buttons
...
const FlowOrchestrator = () => {
  ...
  const [prefetchedRoastData, setPrefetchedRoastData] = useState<
    RoastItem[] | null
  >(null);
  ...
  const prefetchRoastData = useCallback(async (username: string) => {
    ...
    try {
      ...
      console.warn(
        "Could not prefetch roast data, will fetch during ROAST step:",
        error
      );
  ...
  const advanceToNextStep = (data?: string) => {
    ...
    if (data && typeof data === "string" && data.trim()) {
      ...
      if (nextIndex < FLOW_STEPS.length) {
        ...
        if (nextStep === "FILLERS") {
          console.log(
            `Will prefetch data for username "${data}" before showing fillers`
          );
    ...
    const nextStepName =
      nextStepIndex < FLOW_STEPS.length ? FLOW_STEPS[nextStepIndex] : "NONE";
  ...
  return (
    <div className="flow-container">
      ...
      {/* Step 2: Video Intro - Comment this block to skip */}
      {currentStep === "VIDEO_INTRO" && (
        <VideoIntro
          onComplete={(navigateTo) => {
            console.log(
              "VideoIntro onComplete called with navigateTo:",
              navigateTo
            );
      ...
      {currentStep === "GIT_INPUT" && (
        <GitInput
          onAnimationComplete={(username: string) => {
            console.log(
              "GitInput onAnimationComplete called with username:",
              username
            );
            if (username && username.trim()) {
